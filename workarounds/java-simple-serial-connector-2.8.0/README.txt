...
Note that as a workaround to prevent serial port naming collisions, Klipper was configured to create a port in /dev/printer[0-9], which is not a port name that
jssc normally recognizes. Thus, jssc's SerialPortList has a snippet added to the regex pattern to recognize devices named /dev/printer[0-9].

The above modification was written by zhang-benj (Benji Zhang, zhang.benj@northeastern.edu, 2022).

The packaged .jar for jssc will be included in the release on the commport-workaround-jssc-Klipper branch of https://github.com/zhang-benj/Photonic3D. I've not
had any luck compiling the full Photonic3D package, so what I did was build just jssc, extract SerialPortList.class and SerialPortList$1.class, and overwrite
the original copies in the bundled jssc .jar.
